...
TRENDING_TOPICS: list[dict[str, str]] = [
    {"title": "배달 초밥 10개 역대급 논란ㄷㄷ", "body": "배달앱으로 초밥 10개 시켰는데 크기가 손가락만 하고 밥이 덜 익어서 항의했더니 사장 답글이 극혐.", "source": "폴백"},
    {"title": "편의점 진상이 라면 끓이다 벌인 일ㅋㅋ", "body": "편의점 알바 중 손님이 라면 끓이다가 냄비를 그대로 두고 나가버림. CCTV 보니 상습범이었고 결국 출입금지.", "source": "폴백"},
    {"title": "치킨 리뷰 별점 1점 단 손님 결말ㄷㄷ", "body": "치킨집에서 별점 1점 리뷰 남긴 손님이 매일 주문하는 걸 발견. 알고 보니 경쟁 업체 사장이었음.", "source": "폴백"},
    {"title": "택시기사가 손님 신고한 진짜 이유ㄷㄷ", "body": "택시 기사가 손님을 경찰에 신고했는데, 차 안에서 음식 먹고 토하고 도망간 상습범이었음.", "source": "폴백"},
    {"title": "배달 기사가 음식 뱉고 간 실화ㅋㅋ", "body": "배달음식 받았는데 국물에 침 섞인 거 발견. CCTV 확인하니 배달 기사가 음식에 뱉는 모습이 찍힘.", "source": "폴백"},
    {"title": "마트 직원이 고객 쫓아낸 결말ㄷㄷ", "body": "마트에서 손님이 상품 다 뜯어보고 안 사는 걸 반복하다가 직원이 쫓아냄. 손님이 항의했지만 CCTV 보니 상습범.", "source": "폴백"},
    {"title": "PC방 손님이 컴퓨터 부순 이유ㅋㅋ", "body": "PC방에서 손님이 키보드 던지고 모니터 깨뜨림. 알고 보니 게임에서 져서 화풀이한 거였고 수리비 청구됨.", "source": "폴백"},
    {"title": "중국집 배달 1시간 넘게 걸린 실화ㄷㄷ", "body": "짜장면 배달이 1시간 30분 걸려서 항의했더니 배달 기사가 음식 먹고 온 거였음. 사장 반응이 더 충격.", "source": "폴백"},
    {"title": "카페 알바가 손님한테 욕한 결말ㅋㅋ", "body": "카페에서 알바가 손님한테 욕했는데, 손님이 매일 와서 알바 괴롭히는 진상이었음. 결국 사장이 손님 출입금지.", "source": "폴백"},
    {"title": "배달 음식 벌레 나온 사장 반응ㄷㄷ", "body": "배달음식에서 바퀴벌레 나와서 환불 요청했더니 사장이 '원래 들어가는 거'라고 답변. 신고 후 결말.", "source": "폴백"},
    {"title": "식당 직원이 음식 바닥에 떨어뜨린 썰ㅋㅋ", "body": "식당에서 직원이 음식 바닥에 떨어뜨리고 다시 담아서 서빙하는 걸 목격. 항의했더니 사장 반응이 극혐.", "source": "폴백"},
    {"title": "편의점 도시락 유통기한 조작 결말ㄷㄷ", "body": "편의점 알바가 유통기한 지난 도시락 스티커 바꿔 붙이는 걸 목격. 신고했더니 사장까지 연루된 조직적 범죄.", "source": "폴백"},
]
