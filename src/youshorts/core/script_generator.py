...
def select_topic(
    ...
    try:
        ...
        if trend_topics:
            # 이전에 쓴 주제 제외
            available = [t for t in trend_topics if t not in used_topics]
            if available:
...
def _build_prompt(topic: str, angle: str, hook_style: str, style: str) -> str:
    ...
    return f"""너는 한국 20~30대가 진짜 보는 유튜브 쇼츠 작가야.
AI가 만든 티가 나면 안 돼. 사람이 직접 쓴 것처럼 써.

[한국 정서 필수 요소]
- 한(恨): 억울함, 분노, 공감 -> "이게 말이 됨?", "진짜 빡치는 게"
- 정(情): 따뜻함, 감동 -> "근데 여기서 반전이", "눈물 날 뻔"
- 흥(興): 웃음, 신남 -> "ㅋㅋㅋ 미쳤", "개웃기네"
- 끈기: 역전, 성공 -> "결국 해냈음", "소름돋는 반전"
...
[절대 하지 말 것]
- "알고 계셨나요?" 이런 교육방송 투 금지
- "오늘은 ~에 대해 알아보겠습니다" 금지
- "첫째, 둘째, 셋째" 나열식 금지
- 번역체 금지 ("그것은 ~했습니다" 이런 거)
- 위키백과 요약 같은 건조한 정보 나열 금지
- "여러분" 금지 (늙은이 느낌)
- "자, 그럼" 금지
- "정말 놀라운" 금지 (AI 느낌)
- "흥미로운" 금지 (AI 느낌)
...
[반드시 할 것]
- 첫 문장에서 바로 빡치게 / 소름돋게 / 웃기게 시작
- 실제 한국 커뮤니티(에펨코리아, 블라인드, 더쿠) 말투 사용
- "ㅋㅋㅋ", "ㄹㅇ", "개", "미쳤", "실화냐", "소름" 자연스럽게 섞기
- 구체적 숫자, 고유명사, 실제 장소명 넣어서 리얼하게
- 끝에 시청자가 댓글 달고 싶게 만들기 ("이거 나만 몰랐어?" / "님들은?")

[대본 구조 - {template['name']}]
- 0~2초: 훅 (이거 안 보면 손해 급)
- 2~30초: 본론 (반전 하나씩 넣으면서 전개)
- 30~55초: 클라이맥스 (가장 충격적인 부분)
- 55~60초: 댓글 유도 마무리
...
- 문장은 짧게 끊어서 (10자 이내가 이상적)
- 마침표 대신 물음표, 느낌표 많이 사용
...
1. 독창성 (다른 채널에서 못 본 정보):
   - 잘 알려지지 않은 사실이나 데이터를 1개 이상 포함
   - 독특한 비유나 비교를 2개 이상 사용
   - "{angle}" 관점에서 새롭게 해석

2. 크리에이터 개인 의견 (수익창출 핵심):
   - 단순 정보 나열이 아닌 자신만의 해석과 의견 필수
   - "근데 내 생각엔..." / "솔직히 이건..." 스타일

3. 감정 표현 태그 (TTS 감정 변화용):
   - [놀람] 충격적인 내용 앞에
   - [강조] 핵심 포인트에
   - [속삭임] 비밀/꿀팁 느낌에
   - [열정] 흥분되는 내용에
...
6. **구어체 톤 규칙**:
   ...
   - 청자 말걸기 ("여러분 이거 실화임", "이거 듣고 소름 안 돋으면 거짓말")

7. **길이**: 전체 대본 {min_chars}-{max_chars}자. 충분히 길게 써라.
