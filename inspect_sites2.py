    ...
    if links:
        ...
        if body_m:
            ...
            if body_m:
                ...
                print('  Body not found with known patterns')
...
print("="*60)
try:
    r = requests.get('https://bbs.ruliweb.com/best/humor/best', headers=headers, timeout=30)
    r.encoding = 'utf-8'
    links = re.findall(r'href="(https?://bbs\.ruliweb\.com/[^"]*read/\d+)"', r.text)
    if links:
        art_url = links[0]
        print(f'  Fetching: {art_url}')
        r2 = requests.get(art_url, headers=headers, timeout=30)
        r2.encoding = 'utf-8'
        ...
        print('  No article links found on list page')
except Exception as e:
    print(f'  ERROR: {e}')